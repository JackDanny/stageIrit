...
/*
    for(int idxkp1 = 0;idxkp1<keypoints1.size();idxkp1++){
          cout << "keypoints1 "<< idxkp1 << " x=" << keypoints1[idxkp1].pt.x;
          cout << " y=" << keypoints1[idxkp1].pt.y<<endl;
    }
*/

     //cout << "keypoints1: " << keypoints1<<endl; 
...
//on supprime tous les pixels associés au keypoint1 precedent
	...
	float p1y = keypoints1[i].pt.y;
        ...
        float p1xRecal = (p1x*image2.rows)/image1.rows;
        float p1yRecal = (p1y*image2.cols)/image1.cols;
         

	for (int x2 = -rayonDist; x2 < rayonDist + 1; x2++) {
	    for (int y2 = -rayonDist; y2 < rayonDist + 1; y2++) {

		float p2x = p1xRecal + x2;
		float p2y = p1yRecal + y2;
...
//on supprime tous les pixels associés au keypoint1 precedent
	...
	float p1y = pointsy[i].pt.y;
        float p1xRecal=(p1x*image1.rows)/image2.rows;
        float p1yRecal=(p1y*image1.cols)/image2.cols;
...
*/
	    ...
	    //il nous faut comparer les points
            ...
            if( (abs( (best3.pt.x) - (pointsx[i].pt.x) ) <= 1.) && (abs ( (best3.pt.y) - pointsx[i].pt.y) <= 1.)){
